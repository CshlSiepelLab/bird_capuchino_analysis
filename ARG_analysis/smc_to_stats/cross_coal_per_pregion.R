...
if(leftFlankEnd-leftFlankStart + rightFlankEnd-rightFlankStart < flankLen) {
   ...
   for(p in 1:nrow(popPairs)) {
      ...
      quantDiff   <- computeQuantileDiff(peakCCs , flankCCs)
      out_df[1,paste(sep="_",popPairs[p,1], popPairs[p,2])] <- round(quantDiff,digits=2)
      cat("\n  ",paste(sep="_",popPairs[p,1], popPairs[p,2])," : ",quantDiff)
   } # end of for(p)
   cat("\n")
